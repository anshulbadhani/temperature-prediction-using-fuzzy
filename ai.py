...
def define_fuzzy_system(df):
    ...
    output_range = (df['actual_next_avg_temp'].min() * 0.95, df['actual_next_avg_temp'].max() * 1.05)
...
def get_fuzzy_labels(df, fuzzy_vars):
    ...
    for name, var in fuzzy_vars.items():
        ...
        for val in df[name]:
            ...
            memberships = {label: fuzz.interp_membership(var.universe, var[label].mf, val) for label in var.terms}
...
def generate_rules_from_data(df, fuzzy_vars):
    ...
    rule_antecedents = ['min_temp_fuzzy', 'max_temp_fuzzy', 'humidity_fuzzy', 'pressure_fuzzy']
    
    # Use mode to find the most frequent consequent for each antecedent combination
    rule_map = df_fuzzified.groupby(rule_antecedents)['actual_next_avg_temp_fuzzy'].agg(lambda x: x.mode()[0]).reset_index()
...
def evaluate_and_plot(df):
    ...
    plt.plot(df.index, df['actual_next_avg_temp'], label='Actual Next Day Temp', color='blue', alpha=0.8)
    plt.plot(df.index, df['predicted_temp'], label='Predicted Temp', color='red', linestyle='--')
    ...
    corr_df = df[['min_temp', 'max_temp', 'humidity', 'pressure', 'actual_next_avg_temp', 'predicted_temp']]
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        min_temp_var, max_temp_var, humidity_var, pressure_var, next_avg_temp_var = define_fuzzy_system(data)
