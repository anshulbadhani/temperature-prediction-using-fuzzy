...
def main():
    ...
    try:
        ...
        bias = (df["current_avg_temp"] - df["predicted_next_avg_temp"]).mean()
        df["predicted_corrected"] = df["predicted_next_avg_temp"] + bias

        # Show results
        valid_predictions = df['predicted_corrected'].dropna()
        ...
        display_cols = ['min_temp', 'max_temp', 'current_avg_temp', 'humidity', 'pressure', 'predicted_corrected']
        ...
        if len(valid_predictions) > 5:
            ...
            prediction_vs_simple = df['predicted_corrected'] - simple_avg
            ...
            if high_pressure_mask.sum() > 0 and low_pressure_mask.sum() > 0:
                high_pressure_avg = df.loc[high_pressure_mask, 'predicted_corrected'].mean()
                low_pressure_avg = df.loc[low_pressure_mask, 'predicted_corrected'].mean()
    ...
    df = pd.read_csv("data_avg_temp_predictions.csv")

    df["actual_temp"] = df["current_avg_temp"].shift(-1)
    df = df.dropna().reset_index(drop=True)

    plt.figure(figsize=(10, 5))
    plt.plot(df["date"], df["actual_temp"], label="Actual", marker="o")
    plt.plot(df["date"], df["predicted_corrected"], label="Predicted", marker="o")
    plt.xlabel("Date")
    ...
    errors = df["predicted_corrected"] - df["current_avg_temp"]
    ...
    plt.scatter(df["pressure"], df["current_avg_temp"], alpha=0.7)
    plt.xlabel("Pressure (hPa)")
    plt.ylabel("Temperature (°C)")
    plt.title("Pressure vs Temperature")
    plt.show()

    plt.scatter(df["humidity"], df["current_avg_temp"], alpha=0.7, color="green")
    plt.xlabel("Humidity (%)")
    plt.ylabel("Temperature (°C)")
    ...
    sns.heatmap(df[["current_avg_temp", "predicted_corrected", "humidity", "pressure"]].corr(), annot=True, cmap="coolwarm")
